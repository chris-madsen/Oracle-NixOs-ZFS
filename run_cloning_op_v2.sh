...
IMAGE_ID="ocid1.image.oc1.eu-stockholm-1.aaaaaaaaf3cjvwkc6foxrppyxs5qy4gli2wzwmbrlyrm3q24rlda3wmtpgma"

echo ">>> Creating Boot Volume from Image..."
VOL_ID=$(oci bv boot-volume create --availability-domain "$AD" --compartment-id "$COMPARTMENT" --source-image-id "$IMAGE_ID" --size-in-gbs 50 --display-name "$NAME" --wait-for-state AVAILABLE --query 'data.id' --raw-output)
echo "Volume: $VOL_ID"
...
# We use dd to overwrite the Ubuntu image on sdd with ZFS from sdb
